...
const Header = () => {
    ...
    const navItems = [
        ...
        { name: "Core Value", href: "/core-value" },
        ...
        >
            <div className="flex justify-between items-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16">
                ...
                <div className="flex-shrink-0">
                    <Link
                        to="/"
                        className="text-[12px] md:text-[26px] font-bold text-white hover:text-gray-300 transition-all duration-300 ease-out transform hover:scale-105 uppercase"
                ...
                <nav className="hidden md:flex space-x-8">
                    {navItems.map((item) => (
                        <button
                            ...
                            className={`px-3 py-2 text-[14px] font-normal transition-all duration-300 ease-out border-b-2 transform hover:scale-105 ${isActive(item.href)
                ...
                <nav className="px-6 py-8 space-y-2 h-full">
                    {navItems.map((item, index) => (
                        <button
                            key={item.name}
                            onClick={() => handleNavClick(item.href)}
                            className={`block w-full text-left px-4 py-4 text-[16px] font-normal transition-all duration-300 ease-out rounded-lg transform hover:scale-[1.02] hover:translate-x-2 ${isActive(item.href)
